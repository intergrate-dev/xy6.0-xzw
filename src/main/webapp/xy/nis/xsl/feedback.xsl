		...
		<xsl:for-each select="DocItem" >
			<tr>
				...
				<td class="center" style="vertical-align:top;">
					...
					<span class="listDate">
					<xsl:value-of select="feed_phone"/><br/>
					<xsl:value-of select="feed_email"/>
					</span>
				</td>
				<td>
					<xsl:if test="SYS_TOPIC != ''">
						<b><xsl:value-of select="SYS_TOPIC"/></b><br/>
					</xsl:if>
					<xsl:value-of select="feed_content"/>
