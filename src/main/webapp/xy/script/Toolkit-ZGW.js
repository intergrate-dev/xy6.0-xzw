e5.mod("workspace.toolkit",function() {
	var api, toolbarparam,
		...
		showArticleBtns = function(evt) {
			...
			$("body").bind("mousedown", _onArticleBtnsLeave); //鼠标点在其它区域时隐藏写稿操作下拉列表
            $("#toolArticleBtns").bind("mouseleave", _onToolArticleBtnsLeave); //鼠标离开时隐藏写稿操作下拉列表
		},
		_hideArticleBtns = function() {
			$("#toolArticleBtns").hide();
			$("body").unbind("mousedown", _onArticleBtnsLeave);
            $("#toolArticleBtns").unbind("mouseleave", _onToolArticleBtnsLeave);
		},
        ...
        _onToolArticleBtnsLeave = function(event){
            if ($(event.target).closest("#toolArticleBtns").length>0) {
                _hideArticleBtns();
            }
        },
	...
	var _prepareMenu = function() {
			...
			while (true) {
				...
				if (!op1) break;
                //发布库显示当日所有栏目的所有稿件 并且隐藏部分相关操作的按钮 该处隐藏列表稿件右键的部分操作
                ...
                if($("#rs_tree").is(":visible")&&tabName=="Web发布库" && col_tree && col_tree.tree && col_tree.tree.getSelectedNodes().length==0) {
                    if (op1.canMenu && (op1.text=='稿件详情'||op1.text=='修改痕迹'||op1.text=='流程记录')) {
