e5.mod("workspace.doclist",function() {
	...
	var self = {
		...
		callDocList : function(){
			...
			//话题颜色设置为背景颜色start
            if($("#TH_a_color")){
            	var index = $("#doclistframe table th").index($("#TH_a_color"));
            	// console.log(index);
                $("#listing table tr td:nth-child(" +(index+1)+")").each(function(){
                    var color = $(this).text();
                    if(color){
		...
		broadcast : function(){
			...
			if(curName == 'Web发布库'){
				...
				var warning = "";
				$(".doclist tr td:nth-child(5)").mouseenter(function(e){
				...
				$(".doclist tr td:nth-child(5)").mouseleave(function(){
					clearTimeout(warning);
					$(".showDiv").hide();
				});
			}else if (curName == 'App发布库') {
				var wH = $(window).height();
				var warning = "";
				$(".doclist tr td:nth-child(5)").mouseenter(function(e){
					var topH = $(this).offset().top;
...
defineListEvents : function(){
    ...
    objs.each(function(i){
		...
		//!$('#topicLibrary').val()这个条件是为了取消话题库里面的双击事件
		//!$('#Mailsubscriptions').val()这个条件是为了取消邮件订阅里面的双击事件
		if (!this.ondblclick && !$('#topicLibrary').val() && !$('#Mailsubscriptions').val())
