...
(function (factory) {
    ...
    FileInput.prototype = {
        ...
        _setZoomContent: function ($frame, animate) {
            ...
            //start 若图片以.0.jpg或.0为后缀，则去掉这后缀

            var _src=$content.find('img').attr('src'),changeSrc;
            if(_src.substr(-6)=='.0.jpg'){
                changeSrc=_src.substr(0,_src.length-6);
            }
            if(_src.substr(-2)=='.0'){
                changeSrc=_src.substr(0,_src.length-2);
            }
            $content.find('img').attr('src',changeSrc);
