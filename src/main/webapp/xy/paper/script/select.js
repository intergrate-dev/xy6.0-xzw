...
var paper_select = {
		...
		init : function() {
			//按钮的响应事件
			$(".btnColumn").click(paper_select.selectColumn); 	//主栏目
			$("#btnColumn").hide();
			$(".btnColumnRel").click(paper_select.selectColumns);	//关联栏目
			$(".btnPreview").click(paper_select.preview);	//预览
			$("#btnSubmit").click(paper_select.save);				//保存按钮点击事件
			$("#btnCancel").click(e5_form_event.doCancel);		//取消按钮点击事件
			$("#siteSelect").change(paper_select.selectSite); //站点选择变更事件
			
			if( paperInfo.channelCount == 1 ){
			...
			//已签发所有渠道，则不需要“确定”按钮了
			if( paperInfo.channelCount == 0 ){
				$("#btnSubmit").hide();
			}
			
			window.onbeforeunload = e5_form_event.beforeExit;
			
			if(!paperInfo.isSiteSelect)
			paper_select.cookieColumns();
		...
		preview : function(evt) {
			...
			if(colID == ''){
				...
				var dataUrl = "../../xy/article/Preview.do?DocLibID=" + paperInfo.DocLibID
				+ "&DocIDs=" + paperInfo.DocIDs + "&refCol=" + colID + "&ch=" + ch;
		...
		//点击保存按钮
		save : function() {
			var param = paper_select._collectData();
			if (!param) return false;
			
			paper_select._post(param);
		},
...
//					value = (value) ? value + "," + form0[i].value : form0[i].value;
			...
			form['siteID'] = paperInfo.siteID;
			return form;
		},
		_post : function(param) {
			if(!paperInfo.isSiteSelect)
			paper_select._writeCookie();
			//Post方式下参数是字符串形式
			var paramString = JSON.stringify(param);
			
			$.ajax({type: "POST", url: "../../xy/paper/paperSelect.do", async:false, 
				data : {
					"DocIDs" : paperInfo.DocIDs,
					"DocLibID" : paperInfo.DocLibID,
					"paramData" : paramString
				},
				success: paper_select.close,
		...
		selectSite : function(){
			paperInfo.siteID = $("#siteSelect").val();
			$("input[id^='channel']").prop("checked",false);
			$("input[id*='_column']").val("");
		}
};
$(function() {
	paper_select.init();
...
e5_form_event = {
	...
	beforeExit : function(e) {
		var dataUrl = "../../e5workspace/after.do?UUID=" + paperInfo.UUID;
...
function columnClose(filterChecked, checks) {
	...
	var name = paper_select.columnFor;
	
	if (paper_select.columnFor.indexOf("columnRel") > 0 ) {
		// 关联栏目中，自动滤掉主栏目
		var colIDs = paper_select.filterCol($("#" + name.slice(0,-3) + "ID").val(), checks[0]);
		var cols = paper_select.filterCol($("#" + name.slice(0,-3)).val(), checks[1]);

		$("#" + name + "ID").val(colIDs);
		$("#" + name).val(cols);
	} else {
		// 关联栏目中，自动滤掉主栏目
		var colIDs = paper_select.filterCol($("#" + name + "ID").val(), checks[0]);
		var cols = paper_select.filterCol($("#" + name).val(), checks[1]);
