...
var column_form = {
	templateFields : [
		["pa_template", "pa_templateArticle", "pa_templateHome"],
		["pa_templatePad", "pa_templateArticlePad", "pa_templateHomePad"]
	],
	init : function() {
		//站点ID赋值
		...
		$("#pa_templateHome").prop("readonly", true);
		$("#pa_templateHomePad").prop("readonly", true);
	...
	selTemplate : function(evt) {
		...
		column_form.curHome = $(evt.target).attr("home"); //是否点了首页模板

		var nameField = column_form._fieldTemplate(channel, type);
		// 根据类型不同，显示出不同的模板选择树，以组为第一级
		var winId = column_form._getWinId(type, channel);
	...
	cancelTemplate : function(type) {
		var winId = column_form._getWinId(type, 0);
		if (column_form.winTmp[winId]) {
			column_form.winTmp[winId].hide(); // 调用dialog.close()会造成浏览器无响应，因此改成hide
		}
		var winId = column_form._getWinId(type, 1);
		if (column_form.winTmp[winId]) {
	...
	_fieldTemplate : function(channel, type) {
		...
		type = parseInt(type);
		if (column_form.curHome) type = 2;
		
		return column_form.templateFields[channel][type];
	},
	_getWinId : function(type, channel) {
		if (column_form.curHome) type = 2;
		return type + "," + channel;
	},
	// ----以下选择发布规则---
