...
<head>
	...
	<script type="text/javascript">
		var paperInfo= {
			...
			isSiteSelect : <c:out value="${isSiteSelect}"/>
		};
	</script>
	<style>
		body
...
<body>
	...
	<form id="form">
		<c:if test="${siteCount > 0}">
			<div>
				<span class="visa">站点： <select id="siteSelect" name="siteID">
				<c:forEach var="site" items="${siteList}">
					<option value="${site.id}" <c:if test="${site.id==siteID}">selected="selected"</c:if>>${site.name}</option>
				</c:forEach>
				</select>
				</span>
			</div>
