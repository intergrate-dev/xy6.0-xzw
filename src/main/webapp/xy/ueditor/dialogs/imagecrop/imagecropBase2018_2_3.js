...
RemoteImage.prototype = {
    ...
    setPreview: function(){
        ...
        if(ow >= oh){
            ...
            height = ((!ow || !oh) ? preview.offsetHeight : Math.min(ow, preview.offsetHeight));
            height = height + (border * 2) > preview.offsetHeight ? height : (preview.offsetHeight - (border * 2));
...
WaterMark.prototype = {
    ...
    initUploadify: function(){
        ...
        _html.push('<div class="thumbnail" style="width: 110px;height: 100px;margin-bottom: 0px;position:relative;background-color: rgba(204,204,204,0.5);">');
    ...
    wmcloseClick: function () {
        ...
        if (confirm("你确定要删除这个水印文件吗？删除后别人将无法使用！")) {
            ...
            var objTarget = obj.parent().parent().parent();

            //获取删除的chosen
            // obj.parent().parent().parent().click(this.wmImgClick);
            console.log(objTarget);
            debugger;
            objTarget.siblings().removeClass("chosen");
            objTarget.addClass("chosen");
            $.ajax({
                async: true,
                ...
                success: function(data, status){
                    objTarget.hide(500);
                    //给最后一个加入点击事件
                    $("#wmUl li").removeClass("chosen");
                    if(objTarget[0].previousSibling && objTarget[0].previousSibling.children && objTarget[0].previousSibling.children[0].className == 'form-inline'){
                        objTarget.prev().addClass("chosen");
                    }else if(objTarget[0].nextSibling && objTarget[0].nextSibling.children && objTarget[0].nextSibling.children[0].className == 'form-inline'){
                        objTarget.next().addClass("chosen");
                    }else{
                        alert('水印已经全部删除完毕，请点击取消键退出')
                    }
