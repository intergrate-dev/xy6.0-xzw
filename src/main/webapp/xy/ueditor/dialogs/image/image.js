...
//      }
    ...
    function setTabFocus(id) {
        ...
        switch (id) {
            ...
            case 'watermark':
                //setAlign(editor.getOpt('imageManagerInsertAlign'));
                WaterMark = searchImage || new WaterMark();
                break;
        }
    }
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            function addFile(file) {
                ...
                //修复添加两张图片后，又点击“继续添加”按钮，不选择图片然后直接关闭资源管理器会提示错误的bug。
        ...
        getInsertList: function () {
            var i, data, list = [],
                align = getAlign(),//去除图片描述——王华英2015年9月24日15:59:16
    ...
    OnlineImage.prototype = {
        init: function () {
			//编辑器的“插入图片”是单独的对话框，取父的siteID
			var siteID = parent.article.siteID;
			var url = "../../../GroupSelect.do?type=0&editor=1&siteID=" + siteID;
			$("#picFrame").attr("src", url);
        },
		//点“确认”按钮时，读子窗口中图片库选中的图片
        getInsertList: function () {
			var list = [];
			
			//取子窗口中选中的图片
			var param = $("#picFrame")[0].contentWindow.toolbarparam;
			var docIDs = param.docIDs;
			if (!docIDs) return list;
				...
				$.ajax({
					...
					success: function (data) {
						...
						for(var a = 0; a < data.length; a++){
							list.push({
								src: "../../xy/image.do?path=" + data[a].path,
								_src: data[a].path,
    ...
    WaterMark.prototype = {
        ...
        initUploadify: function(){
            ...
            $("#uploadFile").uploadify({
                ...
                /*'buttonImage': "../../../../e5script/jquery/uploadify/image.png",*/
                "buttonText": "",
                'height': 113,
                'width': 113,
                "method": "post",
                'swf': '../../../../e5script/jquery/uploadify/uploadify.swf',
                'uploader': '../../../../xy/ueditor/uploadwatermark.do',
                'auto': true,
                "removeTimeout": 0,
                'fileObjName': 'file',
                ...
                'onUploadSuccess': function(file, data){
                    ...
                    $("#queueLi").hide();
                    _this._addImg( _imgPath);
                }
            });
        },
        _addImg : function(_imgPath){
            var _html = new Array();
            _html.push('<li style="float: left;margin-left: 10px;">');
            _html.push('<div class="form-inline">');
            _html.push('<div class="thumbnail" style="width: 110px;height: 110px;">');
                ...
                success: function(data, status){
                    ...
                    if(data.status == "success"){
                        ...
                        for(var _i = 0, _size = _array.length; _i < _size; _i++){
                            _this._addImg(_this.imgPath + "水印存储;" + _array[_i]);
                        }
                    }else{
                        bResult = false;
                    }
