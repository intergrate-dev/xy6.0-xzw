...
var article_all = {
	...
	init : function() {
        ...
        $("#btnSenIll").click(article_all.senIll);			//敏感词/非法词按钮点击事件
        ...
        $("#btnEditleadTitle").click(article_all.editLeadTitle);		//副题引题样式按钮
	...
	initDateTimePicker : function(){
		...
		//点击定时发布时，触发事件 - 选中，加校验；补选中，不加；
		$("#pubTimer").change(function(){
			if($("#pubTimer").is(":checked")){
				$("#pubTime").addClass("validate[required,custom[dateTimeFormat2]]");
			}else{
				$("#pubTime").removeClass("validate[required,custom[dateTimeFormat2]]");
			}
		});
	},
	// 点击选择主栏目
		...
		if(isSenIll) {
            ...
            if (content != null && content == "") {
                ...
                if ((article.requireSensitive == true || article.requireSensitive == "true"
                    ...
                    article_all.checkSensitive($("#SYS_TOPIC").val(), "标题" ,"isCheck");
                    article_all.checkSensitive(content, "内容" ,"isCheck");
                }
            }
		} else {
	...
	//判断后台是否开启了敏感词或者非法词开关,如果都没用就隐藏按钮
	checkSenIll: function(){
        // 因为现在只对编辑器进行了敏感词检测 所以这里判断是否是普通稿件图片稿件和视频稿件
        if (article.type == "0" || article.type == "1" || article.type == "2")
	...
	save : function() {
        ...
        //app发布库下，检查是否有标题图片
        if (article.ch == "1") {
			var picSmall = channel_frame._imgSrc($("#picSmall").attr("src"));
			var picMiddle = channel_frame._imgSrc($("#picMiddle").attr("src"));
			var picBig = channel_frame._imgSrc($("#picBig").attr("src"));
			if(!picSmall && !picMiddle && !picBig){
                var tSave = confirm("还没有上传标题图片，是否提交？");
                if (!tSave) return false;
			}
        }
	...
	publish : function() {
		...
		// 		var tSave = confirm("还没有上传标题图片，是否发布？");
		// 		if (!tSave) return false;
		// 	}
		// }
		param["toPublish"] = 1;
	...
	close : function(msg) {
		if (msg != "ok") {
			...
			if(article.UUID=="8c8c0e93-6f7f-4b52-8a22-3f69da8cd676"){//话题库-查看话题稿件-修改特殊处理
                window.close();
                return;
            }
			var tool = null;
	...
	openEditDialog : function(){
		...
		} else if ( article_all.editDialogType == 5 ){
            content = $("#a_leadTitle").val();
        }
    ...
    editLeadTitle: function(){
        article_all.editDialogType = 5;
        article_all.openEditDialog();
    },
...
e5_form_event = {
	...
	doExit : function(e) {
		...
		else if(article.UUID="8c8c0e93-6f7f-4b52-8a22-3f69da8cd676") {//话题库-查看话题稿件-修改特殊处理
            $("#btnRefresh").click();
            window.close();
        }else
