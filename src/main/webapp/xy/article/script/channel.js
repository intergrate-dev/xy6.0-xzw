...
var channel_frame = {
	...
	setVideos : function() {
		...
		$.ajax({url: theURL, async:true,
			success: function(datas) {
		        var attPath = datas[0].url;
		        if(attPath.indexOf("http")==0){
	...
	isValid : function() {
		...
		if (article.type == 2) {
            ...
            var a_bigID = $("#a_bigID").val();
			var data = channel_frame._getVideoData();
			if ((!data || data.length == 0)&&(a_bigID==""||a_bigID==null)) {
				alert("百格视频ID不能为空!");
                $("#a_bigID").focus();
				return false;
			}
    ...
    getTopics : function(){
		...
		$("#a_topic li").each(function(){
            ...
            nameDataArray.push($(this).attr("name"));
		})
        var datas = [];
	...
	_getFormData : function() {
        ...
        //添加话题的内容
        formData["a_topics"] = channel_frame.getTopics();
