...
<body>
	<script language="javascript" type="text/javascript" src="script/originalsign.js"></script>
	<div>
		<c:if test="${originaledchsCount > 0}">
			<span class="visa">已签发渠道:</span>
						<c:forEach var="originaledch" items="${originaledchs}">
					<li class="li">
						<label class="label">
							<c:out value="${originaledch.name}"/>
						</label>
						<span>主栏目:</span><span class="link"><c:out value="${originaledch.colName}"/></span>
						<span>关联栏目:</span><span class="link1"><c:out value="${originaledch.colRelName}"/></span> 
					</li>
				</c:forEach>
		</c:if>
	</div>
	...
	<form id="form">
		<div>
			...
			<c:if test="${channelCount == 0}">
				<tr><span class="public">该原稿已签发到所有渠道</span></tr>
			</c:if>
		</div>
	</form>
	<div class="btngroup">
