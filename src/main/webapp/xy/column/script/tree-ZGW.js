var col_tree = {
	...
	_click : function(event, treeId, treeNode, clickFlag) {
		...
		// 判断是否是页面加载时的默认点击
        col_tree.colClick0(event, treeId, treeNode, clickFlag, col_tree.isFirstClick);
		if(col_tree.isFirstClick){
            col_tree.isFirstClick = false;
            col_tree.tree.cancelSelectedNode(treeNode);
        }
	},
	...
	_finding : function() {
		...
		if (col) {
            ...
            if(ids.length > 1) {
                var parent = null;
                for (var i = 0; i < ids.length; i++) {
                    var c = col_tree.tree.getNodeByParam("id", ids[0], parent);
