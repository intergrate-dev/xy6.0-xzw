var tpl_tree={
	...
	type : 0,
	siteID : 1, //当前站点ID
	channel : 0,//栏目发布渠道,0位PC，1为移动
	rootUrl : "../../xy/template/Template.do?",
	tree:null,
	init : function() {
		var theURL = tpl_tree.rootUrl + "&siteID=" + tpl_tree.siteID
										+ "&channel=" + tpl_tree.channel
										+ "&type=" + tpl_tree.type;
	...
	find : function(bID) {
		...
		if (tpl) {	
			var ptpl = tpl_tree.tree.getNodeByParam("id", tpl.pid, null);
			if (ptpl) {
				tpl_tree.tree.expandNode(ptpl,true);
			}
...
tpl_tree.autoCompleter = {
	url : null,
	init : function() {
		tpl_tree.autoCompleter.url = "../../xy/template/Find.do?siteID=" + tpl_tree.siteID
															+ "&channel=" + tpl_tree.channel
															+ "&type=" + tpl_tree.type;
