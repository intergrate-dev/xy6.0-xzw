	...
	<head>
		...
		<script type="text/javascript">
			$(function(){
				...
				//加载文档类型
				//设置更新
				if($.query.get("formName")!=null&&$.query.get("formName")!=""){

					$("#sl-doc-type").val($.query.get("docTypeID"));
					$("#sl-doc-type").attr("disabled","disabled");

					$("#J_FormID").val($.query.get("formID"));
					$("#txt-formname").val($.query.get("formName"));

					$("#txt-csspath").val($.query.get("cssPath"));
					$("#txt-jspath").val($.query.get("jsPath"));
					$("#txt-formcode").val($.query.get("formCode"));

				}
				//设置更新
				$("#formcustom").validationEngine({
					...
					onValidationComplete:function(from,r){
						...
						var valData = {
								...
								"formname":$("#txt-formname").val(),
								"formcode":$("#txt-formcode").val(),
								"doctiypeid":docTypeID
								
						};
						
						//验证是否重复
						$.ajax({
								url: "QueryFormController.do",
								async: false,
								data:valData,
								success: function(data) {
			...
			<div class="field-container">
				<div class="container">
					<table cellpadding="0" cellspacing="0" class="table">
						...
						<tr>
							<td style="text-align:right;"><span class="field-required">*</span><i18n:message key="e5dom.form.custom.formname"/>:</td>
							<td><input id="txt-formname" class="validate[required,custom[onlyCharNumberChiness],maxSize[60]]" type="text" /></td>
						</tr>
						<tr>
							<td style="text-align:right;"><span class="field-required">*</span><i18n:message key="e5dom.form.custom.formcode"/>:</td>
							<td><input id="txt-formcode" class="validate[required,custom[onlyCharNumberChiness],maxSize[60]]" type="text" /></td>
						</tr>
						<tr>
