...
<head>
	...
	<script type="text/javascript">
			...
			$("#cloneForm").validationEngine({
				...
				onValidationComplete:function(from,r){
					if(r){
						...
						var theURL = "FormController.do?action=cloneForm&formID=" + selectedTr + 
		...
		function init(){
			sidebarList = $("#sidebar");
			// var sidebarListTop = sidebarList.offset().top;
			$("body").bind("resize",resizeHandle);
			needResize = 1;
			bodyFrame = $("#bodyFrame")
			browser = $("#browser");
			// $(document).bind("scroll",function(){
			// 	var _top = Math.max(Math.max(document.body.scrollTop,document.documentElement.scrollTop)-sidebarListTop,0)
			// 	sidebarList.css("top",_top);
			// 	browser.css("top",_top);
			// });
		...
		function resizeHandle(){
			...
			var bodyFrameH = bodyH-browser.outerHeight(true) - 20;
		...
		function doBrowser(){
			if(selectedTr){
				bodyFrame.attr({
					...
					"src":"form/Preview.do?action=preView&formID=" + selectedTr
				});
		...
		function doAdd(){
			...
			bodyFrame.attr("src","../e5dom/FormCustom.jsp");
		...
		function doModNew(){
			...
			if(selectedTr){
				//得到现有表单数据
				$.ajax({
					url:"FormController.do?action=getForm&formID="+ selectedTr,
					dataType:"json",
					async:false,
					success:function(data) {
						if(data!=""){
							...
							bodyFrame.attr("src","../e5dom/FormCustom.jsp?"+$.param(formData));
							// $(window).unbind("resize",resizeHandle);
							needResize = 0;
							bodyFrame.height("100%");
						}
		...
		function doDel(){
			if(selectedTr){
				...
				if(ok){
					$.get("FormController.do", { action: "delForm", formID: selectedTr } ,function(){
		...
		function initFormList(){
			var docTypeID;
			$.ajax({
				url:"FormController.do?action=listFormJson", 
				type:"GET",
				dataType:"json",
				async:false, 
				success:function(data) {
		...
		function regOperation(){
			...
			$.ajax({
				  type: "POST",
				  url: "FormController.do?action=registOperate&formID=" + selectedTr + 
		...
		function exportJsp(){
			bodyFrame.attr(
					"src",
					"FormController.do?action=exportToJsp&formID=" + selectedTr);
		}
...
<div class="mainBodyWrap">
	<div id="sidebar" class="sidebar-list">
		<table id="tab0" cellpadding="0" cellspacing="0" class="table">
			<caption><b class="sidebar-list-hide"></b><i18n:message key="e5dom.form.manager"/></caption>
		</table>
	...
	<div class="bodyFrame">
		...
		<table width="100%">
			<tr>
				<td width="50%"><b class="sidebar-list-show"></b><i18n:message key="e5dom.form.custom.preview"/></td>
				...
				<input id="btnExportJsp" class="button" type="button" value="<i18n:message key="e5dom.form.custom.btnexportjsp"/>" onclick="exportJsp();"/>
				</td>
			</tr>
		</table>
		</div>
