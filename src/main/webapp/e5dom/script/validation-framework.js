...
function doValidate(formRef) {
	try {
		...
		if (form != null) {
			...
			var ok = ValidationFramework.validateForm(form);
			return ok;
		} else {
			return false;
		}
...
function getXmlHttpRequest() {
	...
	if (xmlhttp == null) {
		throw ("cannot find a xmlhttp implementation");
	}
...
ValidationFramework.validateDepend = function(field, depend) {
	...
	} else if(depend.getName() == "engAndChnChar") {
		return ValidateMethodFactory.validateEngAndCHNChar(field, depend.getParams());
...
ValidationFramework.getDocumentElement = function() {
	...
	*/
	...
	ValidationFramework._validationCache = xmlhttp.responseXML.documentElement;
	var lang = "auto";
	if (ValidationFramework._validationCache)
		lang = ValidationFramework._validationCache.getAttribute("lang");
...
ValidationFramework.getErrorString = function(field, depend) {
	...
	if (typeof ValidationErrorString[locale] != 'object') {
		stringResource = ValidationErrorString['en_us'];
	} else {
		stringResource = ValidationErrorString[locale];
	}
	...
	if (typeof retStr != 'string') {
		...
		retStr = retStr.replace("{0}", ValidationFramework.getI18nInfoDisplayName(field.getDisplayName()));
		return retStr;
	}
	...
	if (dep == "minlength" || dep == "maxlength" || dep == "date" ) {
		...
		if (eqField == null) {
			...
			retStr = retStr.replace("{1}", ValidationFramework.getI18nInfoDisplayName(field.getForm().findField(depend.getParams()[0]).getDisplayName()));
		}
...
ValidationFramework.getI18nInfoDisplayName = function(displayName) {

//	alert("i18nInformation = "+(typeof i18nInformation));

	var stringResource = null;
	if (typeof i18nInformation[locale] != 'object') {
		stringResource = i18nInformation['en_us'];
	} else {
		stringResource = i18nInformation[locale];
	}

	return stringResource[displayName];
...
ValidateMethodFactory.validateEngAndCHNChar = function(field, params) {
	var obj = ValidationFramework.getWebFormFieldObj(field);
	if (obj == null) return false;
	if (obj.value == "") return true;
	var exp1 = new RegExp("^[A-Za-z0-9_]*$");
	var exp2 = new RegExp("^[\u4E00-\u9FA5\uF900-\uFA2D]*$");
	return (exp1.test(obj.value)) || (exp2.test(obj.value));
}
...
ValidationErrorString["zh_cn"] = new Object();
ValidationErrorString["zh_cn"]["default"]="域{0}校验失败。";
ValidationErrorString["zh_cn"]["required"]="{0}不能为空。";
ValidationErrorString["zh_cn"]["integer"]="{0}必须是一个整数。";
ValidationErrorString["zh_cn"]["double"]="{0}必须是一个浮点数（带小数点）。";
ValidationErrorString["zh_cn"]["commonchar"] = "{0}必须是普通英文字符：字母，数字和下划线。";
ValidationErrorString["zh_cn"]["chinesechar"] = "{0}必须是中文字符。";
ValidationErrorString["zh_cn"]["engAndChnChar"] = "{0}必须是中文或英文字符。";
ValidationErrorString["zh_cn"]["minlength"]="{0}长度不能小于{1}个字符。";
ValidationErrorString["zh_cn"]["maxlength"]="{0}长度不能大于{1}个字符。" ;
ValidationErrorString["zh_cn"]["invalid"]="{0}无效。";                                                             
ValidationErrorString["zh_cn"]["date"]="{0}不是一个有效日期，期待格式：{1}。";
ValidationErrorString["zh_cn"]["integerrange"]="{0}必须在整数{1}和{2}之间。";
ValidationErrorString["zh_cn"]["doublerange"]="{0}必须在浮点数{1}和{2}之间。";
ValidationErrorString["zh_cn"]["pid"]="{0}不是一个有效身份证号。";
ValidationErrorString["zh_cn"]["email"]="{0}不是一个有效的Email。";
ValidationErrorString["zh_cn"]["equalsfield"]="{0}必须和{1}一致。";

////English(en_us)
ValidationErrorString["en_us"] = new Object();
ValidationErrorString["en_us"]["default"]="Failed when validating filed {0}.";
ValidationErrorString["en_us"]["required"]="{0} is required.";
ValidationErrorString["en_us"]["integer"]="{0} must be a integer.";
ValidationErrorString["en_us"]["double"]="{0} must be a double value. ";
ValidationErrorString["en_us"]["commonchar"] = "{0} should be common ascii characters, A-Z,a-z and undercore. ";
ValidationErrorString["en_us"]["chinesechar"] = "{0} must be chinese characters. ";
ValidationErrorString["en_us"]["engAndChnChar"] = "{0}must be chinese or english characters。";
ValidationErrorString["en_us"]["minlength"]="{0} cannot be less then {1}. ";
ValidationErrorString["en_us"]["maxlength"]="{0} cannot be more then {1}. ";
ValidationErrorString["en_us"]["invalid"]="{0} in invalid. ";                                                             
ValidationErrorString["en_us"]["date"]="{0} is not an invalid date format: {1}. ";
ValidationErrorString["en_us"]["integerrange"]="{0} should be between number {1} and {2}. ";
ValidationErrorString["en_us"]["doublerange"]="{0} should be between double {1} and {2}. ";
ValidationErrorString["en_us"]["pid"]="{0} is not an valid pid. ";
ValidationErrorString["en_us"]["email"]="{0} is not an valid email address. ";
ValidationErrorString["en_us"]["equalsfield"]="{0} should be agree with field {1}. ";
