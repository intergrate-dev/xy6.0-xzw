...
<script type="text/javascript" src="./../e5script/jquery/jquery.table2excel.js"></script>
...
<body>
	...
	<div class="wrap" style="margin:-1px; border: none;">
		...
		<li class="channelTab" id="channelTab1" onclick="_tabClick(1)">订单明细</li>
		<li class="channelTab" id="channelTab2" onclick="_tabClick(2)">支付交易管理</li>
	</ul>
	...
	<iframe id="contentFrame1" name="contentFrame1" src="MainSimple.do?t=paylog&siteID=<%=request.getParameter("siteID")%>" frameborder="0" scrolling="auto" width="100%" height="90%"></iframe>	
</body>
	...
	function search(){
		...
		function loaddata(fin){
			$.ajax({
				cache: true,
				type: "POST",
				url:"<%=request.getContextPath()%>/amuc/setmeal/Static.do?a=findorder&beginTime="+$("#beginTime").val()+"&endTime="+$("#endTime").val()+"&siteID="+siteID,
		...
		function pandect(){
			...
			var orderdata= JSON.parse($("#orderdate").val());
					$("#mytable").html("")
					$("#mytable").append("<tr><td width='100px'>日期</td><td  width='100px'>套餐金额</td><td  width='100px'>套餐数</td></tr>")
						if(orderdata.data.length>0){
							...
							$("#mytable").append("<tr><td>合计</td><td>￥"+numberWithCommas(TotalMoney.toFixed(2))+"</td><td>"+Totality+"</td></tr>")
						 }
		...
		$.ajax({
				cache: true,
				type: "POST",
				url:"<%=request.getContextPath()%>/amuc/setmeal/Static.do?a=findAllSetMeal&siteID="+siteID,
					success : function(data) {
						...
						if (data.code == 0) {
							...
							for ( var i = 0; i < data.length; i++) {
								...
								str1 += "<td colspan='2'  width='180px'>" + data[i].name
										+ "</td>";
								str2 += "<td width='90px'>订单金额</td><td width='90px'>订单总数</td>";

								data[i].money = 0
								data[i].number = 0
							}
							str1 += "<td colspan='2'  width='180px'>小计</td></tr>";
							str2 += "<td width='90px'>订单金额</td><td width='90px'>订单总数</td></tr>";
							$("#mytable").append(str1);
							$("#mytable").append(str2);
							...
							var he = "<tr><td>合计</td>"
							for ( var i = 0; i < data.length; i++) {
								he += "<td>￥" + numberWithCommas(data[i].money.toFixed(2)) + "</td><td>"
										+ data[i].number + "</td>"

							}
							he += "<td>￥" + numberWithCommas(TotalMoney.toFixed(2)) + "</td><td>" + Totality
									+ "</td></tr>"
							$("#mytable").append(he);
						} else {
							alert("查询失败");
