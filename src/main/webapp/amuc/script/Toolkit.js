e5.mod("workspace.toolkit",function() {
	var api, toolbarparam, 
		...
		parseOperation = function(otr, newop, i/*, opCount_perRow*/){
			...
			//console.log("Toolkit.js parseOperation return")
			if(localStorage.hasOwnProperty('inviType')
				&& localStorage.getItem('inviType') == 1){
				if($(".channelTab:first").hasClass("select") && newop.text == '修改单位'){
					return;
				}
				if(!$(".channelTab:first").hasClass("select") && newop.text != '修改单位'){
					return;
	...
	var _prepareMenu = function() {
		...
		_showRMenu = function(msgId, msgName, data) {
			...
			menuHeight = menuHeight > 10? menuHeight : 30;
