e5.mod("workspace.doclist",function() {
		...
		*/
		...
		exportList : function(){
			...
			var theURL = "../e5workspace/listExcelExport.do?DocLibID=" + self.statusparam.docLibID
				...
				+ "&ListPage=" + listPage
				+ "&CurrentPage="+parseInt(self.statusparam.currentPage+1)
	...
	var scroll2page = {
		...
		scrollCustomListpage : function(data) {
			...
			//截取实际行，附加到table末尾
			data = table.find("tr").not("tr:first");
...
setListData : function(data) {
	...
	self.bianli(self.statusparam.listPage);
},
...
pinHeader : function() {
	...
	var tHead = listHead.find("tr:first").children();
	thead.find("th").each(function(i,elm){
		var w = parseInt(this.style.width)||this.getAttribute("width");
		
		if(!w)return;
		var nowtd = tbodyHead.eq(i);
		var nowth = tHead.eq(i);
		//获取手机号、移动电话等敏感信息在列表中的位置
		if(nowth.attr("id")=="TH_mMobile"){
			handlerParam.Num = i;
		}
		if(nowth.attr("id")=="TH_maMemberMobile"){
			handlerParam.Num = i;
		}
		if(nowth.attr("id")=="TH_rMemberMobile"){
			handlerParam.Num = i;
		}
		if(nowth.attr("id")=="TH_mgMobile"){
			handlerParam.Num = i;
		}
		//固定电话脱敏显示
		if(nowth.attr("id")=="TH_mPhone"){
			handlerParam.Num1 = i;
		}
		//微信抽奖 开始时间、截止时间、创建时间(开始时间、结束时间、参与时间有可能在同一个列表中，因此用多个全局变量)
		if(nowth.attr("id")=="TH_G_StartTime"){
			handlerParam.Num2 = i;
		}
		if(nowth.attr("id")=="TH_G_EndTime"){
			handlerParam.Num3 = i;
		}
		if(nowth.attr("id")=="TH_SYS_CREATED"){
			handlerParam.Num4 = i;
		}
		if(nowth.attr("id")=="TH_R_StartTime"){
			handlerParam.Num5 = i;
			
		}
		if(nowth.attr("id")=="TH_R_EndTime"){
			handlerParam.Num6 = i;
...
bianli : function(listPage){
	var len = $("#listing").children("table").children("tbody").children("tr").length;
	for(var i=0;i<len;i++){
		var td_0 = $("#listing").children("table").children("tbody").children("tr").eq(i).children("td").eq(0).text();
		
		if(listPage != 14){
			//使手机号脱敏显示
			var mobileNo = $("#listing").children("table").children("tbody").children("tr").eq(i).children("td").eq(handlerParam.Num).text();
			var lenMob = mobileNo.length;
			var strMob = "";
			for(var m=0;m<lenMob-7;m++){
				strMob+="*";
			}
			var phoneNum = mobileNo.substring(0,3) + strMob + mobileNo.substring(lenMob-4,lenMob);
			$("#listing").children("table").children("tbody").children("tr").eq(i).children("td").eq(handlerParam.Num).text(phoneNum);
		}
		
		//使固定电话脱敏显示
		var mobileNo1 = $("#listing").children("table").children("tbody").children("tr").eq(i).children("td").eq(handlerParam.Num1).text();
		var lenMob1 = mobileNo1.length;
		var strMob1 = "";
		for(var n=0;n<lenMob1-5;n++){
			strMob1+="*";
		}
		var phoneNum1 = mobileNo1.substring(0,3) + strMob1 + mobileNo1.substring(lenMob1-2,lenMob1);
		$("#listing").children("table").children("tbody").children("tr").eq(i).children("td").eq(handlerParam.Num1).text(phoneNum1);
		
		//日期
		//开始日期、去掉秒
		var sTime = $("#listing").children("table").children("tbody").children("tr").eq(i).children("td").eq(handlerParam.Num2).text();
		sTime = sTime.substring(0,16);
		$("#listing").children("table").children("tbody").children("tr").eq(i).children("td").eq(handlerParam.Num2).text(sTime);
		//截止日期、去掉秒
		var eTime = $("#listing").children("table").children("tbody").children("tr").eq(i).children("td").eq(handlerParam.Num3).text();
		eTime = eTime.substring(0,16);
		$("#listing").children("table").children("tbody").children("tr").eq(i).children("td").eq(handlerParam.Num3).text(eTime);
		//参与时间、去掉秒
		var cTime = $("#listing").children("table").children("tbody").children("tr").eq(i).children("td").eq(handlerParam.Num4).text();
		cTime = cTime.substring(0,16);
		$("#listing").children("table").children("tbody").children("tr").eq(i).children("td").eq(handlerParam.Num4).text(cTime);
		
		//红包活动去掉秒
		var startTime = $("#listing").children("table").children("tbody").children("tr").eq(i).children("td").eq(handlerParam.Num5).text();
		startTime = startTime.substring(0,16);
		$("#listing").children("table").children("tbody").children("tr").eq(i).children("td").eq(handlerParam.Num5).text(startTime);
		
		//红包活动去掉秒
		var endTime = $("#listing").children("table").children("tbody").children("tr").eq(i).children("td").eq(handlerParam.Num6).text();
		endTime = endTime.substring(0,16);
		$("#listing").children("table").children("tbody").children("tr").eq(i).children("td").eq(handlerParam.Num6).text(endTime);
		
		//复写第一个td中的值
		var td0 = $("#listing").children("table").children("tbody").children("tr").eq(i).children("td").eq(0).text(td_0);
	}
},
showCustomListpage : function() {
...
dClickDoc : function(src, evt){
	...
	var a = self.getClickRoot(src);
		...
		if(parseInt(a.getAttribute("libid"))+2==93){
			return;
		}
...
updateColor : function(a/*, abgColor, afgColor*/){
	...
	for (var i = 0;i < tds.length;i++){
		tds[i].bgColor = abgColor;
		tds[i].style.color = afgColor;
	}*/
	var elm = $(a);
	if(elm.hasClass("isSelect")){
		elm.removeClass("isSelect");
	}else{
		elm.addClass("isSelect");
	}
},
//取消所有选中
...
//------------------------------------
	...
	var init = function(sandbox){
			...
			self.api.listen("workspace.toolkit:refresh_oms", self.refreshListening);
			
			self.api.listen("workspace.toolkit:refreshList",self.refreshListening);
			//发出消息表示加载完毕ready
